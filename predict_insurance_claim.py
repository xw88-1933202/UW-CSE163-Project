...
def fit_and_predict_degrees_tree(df: pd.DataFrame) -> None:
    ...
    print('Tree MSE test set:',
          round(mean_squared_error(labels_test, predictions), 2))
...
def fit_and_predict_degrees_linear(df: pd.DataFrame) -> None:
    ...
    y = df['claim']
    X_numerical = df.drop(['claim', 'gender', 'diabetic', 'smoker', 'region'],
                          axis=1).astype('float64')
    list_numerical = X_numerical.columns
    X = pd.concat([X_numerical,
                  dummies[['gender_female', 'diabetic_No', 'smoker_No',
                           'region_northeast', 'region_northwest',
                           'region_southeast']]], axis=1)
    ...
    print('best lasso model test set mse:',
...
if __name__ == '__main__':
    ...
    [(220.67777789171802, 'age'),
    (1973.9114357246187, 'bmi'),
    (2578.4219804815343, 'bloodpressure'),
    (696.9285432747407, 'children'),
    (-0.0, 'gender_female'),
    (15.758597983817875, 'diabetic_No'),
    (-20670.70474042754, 'smoker_No'),
    (1918.4386309856318, 'region_northeast'),
    (0.0, 'region_northwest'),
    (-477.0371559518968, 'region_southeast')]
